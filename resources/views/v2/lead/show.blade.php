    ...
    <div class="for-slider-main-banner">
        @switch($user_role_id)
            @case(2)
                ...
                @php
                    $invoices = \App\Models\Invoice::where('client_id', $lead->id)->orderBy('created_at', 'DESC')->get();
                @endphp
                @if(count($invoices))
                ...
                @php
                    $lead_user = \App\Models\User::where('client_id', $lead->id)->first();
                    $projects = $lead_user ? $lead_user->recent_projects : [];
                @endphp
                @if(count($projects))
                    <section class="list-0f">
                        <div class="container">
                                        ...
                                        <table>
                                            ...
                                            @foreach($projects as $project)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{str_replace($lead->name, '', str_replace(' - ', ' ', $project->name))}}</td>
                ...
                @php
                    $pending_projects = $lead->user ? get_pending_projects($lead->user->id) : [];
                @endphp
                ...
                @php
                    $briefs_pendings = $lead->user ? get_briefs_pending($lead->user->id) : [];
                @endphp
