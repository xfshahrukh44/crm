...
<div class="breadcrumb row">
    ...
    <div class="col-md-6 text-right">
        <a href="{{ route('admin.lead.create') }}" class="btn btn-primary">Create Lead</a>
    </div>
</div>
...
<div class="row mb-4">
    <div class="col-md-12">
        <div class="card text-left">
            <div class="card-body">
                <form action="{{ route('admin.lead.index') }}" method="GET">
                    <div class="row">
                ...
                <div class="table-responsive">
                    <table class="display table table-striped table-bordered"  style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$datas->id}}</td>
                                <td><a href="{{ route('admin.lead.show', $datas->id) }}">{{$datas->name}} {{$datas->last_name}}</a></td>
                                <td>{{$datas->email}}</td>
                                <td>
                                ...
                                <td>
                                    {{ ($datas->assigned_to->name ?? '') . ' ' . ($datas->assigned_to->last_name ?? '') }}
                                </td>
                                <td>
                                    <a href="{{ route('admin.lead.edit', $datas->id) }}" class="btn btn-primary btn-icon btn-sm">
                                        <span class="ul-btn__icon"><i class="i-Edit"></i></span>
                                        <span class="ul-btn__text">Edit</span>
                                    </a>
                                    <a href="{{ route('admin.lead.show', $datas->id) }}" class="btn btn-dark btn-icon btn-sm">
                                        <span class="ul-btn__icon"><i class="i-Eyeglasses-Smiley"></i></span>
                                        <span class="ul-btn__text">View</span>
                                    </a>
                                    <form method="POST" action="{{route('admin.lead.destroy', $datas->id) }}">
                                        {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
