    ...
    <div class="col-md-12">
        <div class="card mb-4">
            <div class="card-body">
                ...
                <form class="form" action="{{route('admin.user.qa.update', $data->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 form-group mb-3">
                            <label for="category">Brand Name <span>*</span></label>
                            <select name="category[]" id="category" class="form-control select2" required multiple="multiple">
                                @foreach ($category as $categories)
                                    <option value="{{ $categories->id }}" {{ isset($data) && in_array($categories->id, $data->category()->pluck('id')->toArray()) ? 'selected' : '' }}> {{ $categories->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 form-group mb-3">
                        ...
                        <div class="col-md-6 form-group mb-3">
                            ...
                            <select name="is_support_head" id="role" class="form-control">
                                <option value="0" {{ $data->is_support_head == 0 ? 'selected' : '' }}>QA member</option>
                                <option value="1" {{ $data->is_support_head == 1 ? 'selected' : '' }}>QA lead</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-12">
                            <button class="btn btn-primary" type="submit">Update QA</button>
                        </div>
                    </div>
                </form>
            </div>
