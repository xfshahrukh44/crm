...
<div class="row mb-4">
    <div class="col-md-12">
        <div class="card text-left">
            <div class="card-body">
                <form action="{{ route('billing.invoice') }}" method="GET">
                    <div class="row">
...
{{--                                    <span class="btn btn-{{ App\Models\Invoice::STATUS_COLOR[$datas->payment_status] }} btn-sm">--}}
                                    <span class="">
                                        ...
                                        <form method="post" action="{{ route('billing.invoice.paid', $datas->id) }}">
                                            @csrf
                                ...
                                <td>
                                    <div>
                                        @if($datas->payment_status == 1)
                                        <a href="{{ route('billing.invoice.edit', $datas->id) }}" class="mb-2 btn btn-primary btn-icon btn-sm mr-1">
                                            <span class="ul-btn__icon"><i class="i-Edit"></i></span>
                                            <span class="ul-btn__text">Edit</span>
                                        </a>
                                        @endif
                                        <a href="{{ route('billing.link', $datas->id) }}" class="btn btn-info btn-icon btn-sm">
    ...
    $('.auth-create').on('click', function () {
        ...
        if(auth == 0){
            swal({
                ...
                $.ajax({
                    type:'POST',
                    url: "{{ route('billing.client.createauth') }}",
            ...
            swal({
                ...
                $.ajax({
                    type:'POST',
                    url: "{{ route('billing.client.updateauth') }}",
                    data: {id: id, pass:inputValue},
                    success:function(data) {
                        if(data.success == true){
                            swal("Auth Created", "Password is : " + inputValue, "success");
